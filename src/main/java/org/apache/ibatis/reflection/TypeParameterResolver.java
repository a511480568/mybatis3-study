...
public class TypeParameterResolver {
  ...
  private static Type resolveType(Type type, Type srcType, Class<?> declaringClass) {
    // 如果是类型变量，比如 T, Map<K,V> 中 T K V 就是类型变量
    if (type instanceof TypeVariable) {
      return resolveTypeVar((TypeVariable<?>) type, srcType, declaringClass);
    }
    // 如果是参数化类型，比如 List<T> 中 T 就是参数化类型,这个和上面的类型变量的区别是这个是参数化的类型
    if (type instanceof ParameterizedType) {
      return resolveParameterizedType((ParameterizedType) type, srcType, declaringClass);
      // 如果是泛型数组类型，比如 T[] 中 T 就是泛型数组类型
  ...
  private static ParameterizedType resolveParameterizedType(ParameterizedType parameterizedType, Type srcType,
      Class<?> declaringClass) {
    // 原始变量类型，比如 List<T> 中 List 就是原始变量类型
    Class<?> rawType = (Class<?>) parameterizedType.getRawType();
    // 获取类型参数，比如上面的 List<T> 中 T 就是类型参数
