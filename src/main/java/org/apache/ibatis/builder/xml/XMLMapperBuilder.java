...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  private void configurationElement(XNode context) {
    try {
      ...
      // 这个是解析 Mapper.xml 文件中的 <sql></sql> 标签的，具体可以参考 userMapper.xml 文件
  ...
  private void sqlElement(List<XNode> list, String requiredDatabaseId) {
    for (XNode context : list) {
      ...
      // 这里获取的是 <sql id = "base_column_list"></sql> 标签中的 id，然后调用 builderAssistant.applyCurrentNamespace 方法只是需要将这个 id 给扩充一下
      // 扩充后的 id = namespace.id (namespace 是 mapper.xml 中的 namespace)
  ...
  private void bindMapperForNamespace() {
    ...
    if (namespace != null) {
      ...
      if (boundType != null && !configuration.hasMapper(boundType)) {
        ...
        // configuration 中的 mapperRegistry 中保存了 namespace -> mapperClass 的映射，后续通过 mapperProxyFactory 创建代理对象
