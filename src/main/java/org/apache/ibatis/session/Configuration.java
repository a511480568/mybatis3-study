 ...
 * 该类包含了 mybatis-config.xml 配置中各个标签的属性，以及全局属性。
 */
public class Configuration {
  ...
  // 当 mybatis 查询数据库并将结果映射到 java 对象时，使用 ObjectFactory 创建对象实例，并使用 ObjectWrapperFactory 获取包装器来设置对象属性
  // 这两个工厂接口体现了 Mybatis 设计中的开闭原则，允许用户通过自定义实现扩展 Mybatis 的对象创建和属性访问行为，而不需要修改 Mybatis 核心代码
  ...
  // 用于注册 Mapper 接口信息，建立 Mapper 接口的 Class 对象和 MapperProxyFactory 的关系，其中 MapperProxyFactory 对象用于创建 Mapper 动态代理对象
  ...
  // 该对象描述 <insert|select|update|delete> 标签信息或者通过 @Insert|@Select|@Update|@Delete 注解描述
  // Mybatis 将所有的 MappedStatement 对象注册到该属性中，其中 key 为 Mapper 的 id，value 为 MappedStatement 对象
  protected final Map<String, MappedStatement> mappedStatements = new StrictMap<MappedStatement>(
      "Mapped Statements collection")
          .conflictMessageProducer((savedValue, targetValue) -> ". please check " + savedValue.getResource() + " and "
              + targetValue.getResource());
  protected final Map<String, Cache> caches = new StrictMap<>("Caches collection");
  // 用于注册 Mapper 配置文件中 <resultMap> 标签信息，result Map 用于建立 java 实体属性和数据库字段之间的映射关系，
  // 其中 key 为 resultMap 的 id，value 为 resultMap 对象
  protected final Map<String, ResultMap> resultMaps = new StrictMap<>("Result Maps collection");
  // 用于注册 Mapper 中通过 <parameterMap> 标签注册的参数映射信息，其中 key 为 parameterMap 的 id，value 为 parameterMap 对象
  ...
  /**
   * 创建 executor，通过 executorType 创建，默认是 SimpleExecutor，如果开启了二级缓存，则创建 CachingExecutor
   */
